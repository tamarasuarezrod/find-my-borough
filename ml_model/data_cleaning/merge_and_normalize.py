...
def merge_all_features(df_rent, df_crime, df_age, df_zone):
    df = pd.merge(df_rent, df_crime, on="borough", how="inner")
    df = pd.merge(df, df_age, on="borough", how="inner")
    ...
    df["norm_safety"] = 1 - (df["total_crimes"] - crime_min) / (crime_max - crime_min)
    ...
    df = df[["borough", "norm_rent", "norm_safety", "norm_youth", "norm_centrality"]]
